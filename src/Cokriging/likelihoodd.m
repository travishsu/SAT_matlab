...
%     LnDetPsidXe = log(det(PsidXe));
% 
%     % Difference vector
%     d = ye-rho.*yc(end-ne+1:end);
% 
%     % Use back-substitution of Cholesky instead of inverse
%     mud = (one'*(PsidXe\d))/(one'*(PsidXe\one));
%     SigmaSqrd = (d-one.*mud)'*(PsidXe\(d-one.*mud))/ne;
%     NegLnLiked = -1*(-(ne/2)*log(SigmaSqrd)-0.5*LnDetPsidXe);
    ...
    for j = 1:ne
        psi = zeros(nc, 1);
        for i = 1:nc
            psi(i) = exp(-sum(thetac.*(Xe(j, :)-Xc(i, :)).^2));
        end
        PsicXc = ModelInfo.PsicXc;
        muc    = ModelInfo.muc;
        ycxe(j) = muc+psi'*(PsicXc\(yc-muc*ones(nc, 1)));
    end
